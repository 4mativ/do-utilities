...
def batch_distance_request_coords(
...
def batch_routed_distance(df, address_column1, address_column2, distance_column, route, navigation_mode):
    os.chdir(os.path.dirname(__file__))
    files = glob("/output/*")
    for f in files:
        os.remove(f)
    ...
    return batch_distance_requests(
        df, address_pairs, address_column1, address_column2, distance_column, route, navigation_mode
    )


def batch_routed_distance_coords(df, lat_col1, lon_col1, lat_col2, lon_col2, distance_column,
    ...
    return batch_distance_request_coords(
        df, address_pairs, lat_col1, lon_col1, lat_col2, lon_col2, distance_column, route, navigation_mode
    )


def batch_geocode_df(df, address_columns, parallel=False, save=True):
    os.chdir(os.path.dirname(__file__))
    files = glob("/output/*")
    for f in files:
        os.remove(f)
    ...
    if parallel:
        geocodes = batch_geocode(addresses)
    else:
...
def batch_reverse_geocode(df, lat_col, long_col, new_address_col):
    os.chdir(os.path.dirname(__file__))
...
def batch_geocode_lookup(df, address_column_to_lat_long_dict):
